...
# 1. Initialize video capture with your local data directory
cap = cv2.VideoCapture("data/video.mp4")
assert cap.isOpened(), "Error reading video file"

# Define coordinates for the queue monitoring area
queue_region = [(20, 400), (1080, 400), (1080, 360), (20, 360)]  # Rectangular region points
# queue_region = [(20, 400), (1080, 400), (1080, 360), (20, 360), (20, 400)]  # Polygon region points fallback

# Get original video properties
w, h, fps = (int(cap.get(x)) for x in (cv2.CAP_PROP_FRAME_WIDTH, cv2.CAP_PROP_FRAME_HEIGHT, cv2.CAP_PROP_FPS))

# 2. Initialize video writer
video_writer = cv2.VideoWriter("queue_management_output.avi", cv2.VideoWriter_fourcc(*"mp4v"), fps, (w, h))

# 3. Initialize Queue Manager object (Set show=False to prevent window conflicts)
queuemanager = solutions.QueueManager(
    model="model/yolov8n.pt",  # Updated path to your default YOLO model directory
    region=queue_region,      # Pass the queue region tracking points
    show=False,               # Disable built-in window to allow full resizing control
)

# --- WINDOW CONFIGURATION (ALLOW RESIZING VIA MOUSE) ---
win_name = "YOLO Real-Time Queue Management"
...
while cap.isOpened():
    ...
    # Process frame and update queue counting calculations
